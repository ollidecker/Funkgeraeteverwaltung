...
def get_version() -> str:
    try:
        return VERSION_FILE.read_text(encoding="utf-8").strip() or "unbekannte Version"
    except Exception:
...
def template_context(request: Request, **kwargs):
    ctx = {"request": request, "version": get_version()}
    ctx.update(kwargs)
    return ctx
...
def init_db():
    with db() as con:
        ...
        for p in projects:
            rows = con.execute(
                "SELECT id FROM loans WHERE project_id=? ORDER BY created_at ASC, id ASC",
                (p["id"],),
            ).fetchall()
            for idx, row in enumerate(rows, start=1):
                con.execute(
                    "UPDATE loans SET sequence_no=COALESCE(sequence_no, ?) WHERE id=?",
                    (idx, row["id"]),
                )
...
def loan_items(loan) -> List[str]:
    items = [
        f"Funkgerät Seriennummer {loan['radio_no']}",
        f"Batterie Seriennummer {loan['battery_no']}",
    ]
...
def ensure_unique_serials(project_id: int, serials: List[tuple], exclude_loan_id: Optional[int] = None):
    ...
    for label, serial in serials:
        ...
        if key in seen_in_form:
            raise HTTPException(
                400,
                f"Seriennummer doppelt in dieser Eingabe: {serial} ({label}) ist identisch mit {seen_in_form[key]}.",
            )
    ...
    with db() as con:
        rows = con.execute(
            "SELECT * FROM loans WHERE project_id=? AND status IN ('open','partial')",
            (project_id,),
        ).fetchall()
    ...
    for loan in rows:
        ...
        for item in loan_items(loan):
            ...
            if serial:
                active_serials[normalize_serial(serial)] = (
                    f"{serial} bei Ausleihe #{loan['sequence_no']} / {loan['name']}"
                )
    ...
    for label, serial, key in cleaned:
        if key in active_serials:
            raise HTTPException(
                400,
                f"Seriennummer bereits aktiv vergeben: {serial} ({label}). "
                f"Bereits verwendet: {active_serials[key]}. Erst zurückgeben, dann erneut ausgeben.",
            )
...
def add_signature(story, signature_data: str, st):
    ...
    story.append(Paragraph("Unterschrift:", st["Line"]))
...
def para_lines(story, lines, st):
    for line in lines:
        if line == "":
            ...
            story.append(Paragraph(escape(line), st["Line"]))
        else:
            story.append(Paragraph(escape(line), st["Line"]))
...
def index(request: Request):
    ...
    return templates.TemplateResponse(
        "index.html",
        template_context(request, projects=projects),
    )
...
async def import_project(file: UploadFile = File(...)):
    ...
    with db() as con:
        ...
        if imported_logo:
            ...
            con.execute(
                "UPDATE projects SET pdf_logo_filename=? WHERE id=?",
                (logo_name, new_pid),
            )
        ...
        for loan in data.get("loans", []):
            con.execute(
                """
                INSERT INTO loans(
                    project_id,name,date,radio_no,battery_no,accessories_json,
                    custom_accessory,signature_data,return_json,return_note,status,
                    created_at,updated_at,sequence_no,extra_accessories_json,
                    closed_at,return_history_json
                )
                VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)
                """,
                (
                    ...
                    loan["radio_no"],
                    loan["battery_no"],
                    loan.get("accessories_json", "[]"),
                    loan.get("custom_accessory", ""),
                    loan.get("signature_data", ""),
                    loan.get("return_json", "[]"),
                    loan.get("return_note", ""),
                    loan.get("status", "open"),
                    loan.get("created_at", now()),
                    loan.get("updated_at", now()),
                    loan.get("sequence_no"),
                    loan.get("extra_accessories_json", "[]"),
                    loan.get("closed_at"),
                    loan.get("return_history_json", "[]"),
                ),
            )
...
def project(request: Request, project_id: int):
    ...
    with db() as con:
        loans = con.execute(
            "SELECT * FROM loans WHERE project_id=? ORDER BY sequence_no DESC, created_at DESC",
            (project_id,),
        ).fetchall()

    return templates.TemplateResponse(
        "project.html",
        template_context(request, project=proj, loans=loans),
    )
...
def create_loan(
    ...
    radio_no: str = Form(...),
    battery_no: str = Form(...),
    accessories: List[str] = Form([]),
    extra_battery_no: str = Form(""),
    custom_accessory: str = Form(""),
    ...
    with db() as con:
        next_seq = con.execute(
            "SELECT COALESCE(MAX(sequence_no), 0) + 1 AS n FROM loans WHERE project_id=?",
            (project_id,),
        ).fetchone()["n"]
        ...
        cur = con.execute(
            ...
            INSERT INTO loans(
                project_id,name,date,radio_no,battery_no,accessories_json,
                custom_accessory,signature_data,return_json,status,created_at,
                updated_at,sequence_no,extra_accessories_json
            )
            VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?)
            """,
            (
                ...
                radio_no.strip(),
                battery_no.strip(),
                json.dumps(accessory_list, ensure_ascii=False),
                custom_accessory.strip(),
                signature_data,
    ...
    log(
        project_id,
        (
            f"AKTION: Ausgabe angelegt\n"
            f"Ausleihe: #{next_seq}\n"
            f"Name: {name}\n"
            f"Datum: {fmt_date(date)}\n"
            f"Funkgerät Seriennummer: {radio_no}\n"
            f"Batterie Seriennummer: {battery_no}\n"
            f"Zubehör: {', '.join(accessory_list + ([custom_accessory] if custom_accessory else [])) or '-'}"
        ),
    )
...
def loan_detail(request: Request, loan_id: int):
    ...
    return templates.TemplateResponse(
        ...
        template_context(
            ...
            project=proj,
            loan=loan,
            items=loan_items(loan),
            returned=returned,
            missing=missing,
            accessories=ACCESSORIES,
            return_history=json.loads(loan["return_history_json"] or "[]"),
        ),
    )
...
def add_items(
    ...
    if "Spare Batterie" in accessories:
        ensure_unique_serials(
            loan["project_id"],
            [("Spare Batterie Seriennummer", extra_battery_no)],
    ...
    with db() as con:
        con.execute(
            "UPDATE loans SET extra_accessories_json=?, status=?, updated_at=? WHERE id=?",
            (json.dumps(combined, ensure_ascii=False), status, now(), loan_id),
        )

    log(
        loan["project_id"],
        (
            f"AKTION: Nachträgliche Ausgabe\n"
            f"Ausleihe: #{loan['sequence_no']}\n"
            f"Name: {loan['name']}\n"
            f"Neu ausgegeben: {', '.join(new_items)}"
        ),
    )
...
def save_return(
    loan_id: int,
    returned_items: List[str] = Form([]),
    return_note: str = Form(""),
):
    loan = get_loan(loan_id)
    ...
    if newly_returned or return_note.strip():
        history.append(
            {
                "timestamp": timestamp,
                "items": newly_returned,
                "all_returned_after_save": returned_items,
                "note": return_note.strip(),
            }
        )
    ...
    with db() as con:
        con.execute(
            """
            UPDATE loans
            SET return_json=?, return_note=?, status=?, closed_at=?, updated_at=?, return_history_json=?
            WHERE id=?
            """,
            (
                json.dumps(returned_items, ensure_ascii=False),
                return_note.strip(),
                status,
                closed_at,
                timestamp,
    ...
    log(
        loan["project_id"],
        (
            f"AKTION: Rückgabe gespeichert\n"
            f"Ausleihe: #{loan['sequence_no']}\n"
            f"Name: {loan['name']}\n"
            f"Zeitpunkt: {fmt_datetime(timestamp)}\n"
            f"Neu zurück: {', '.join(newly_returned) or '-'}\n"
            f"Zurück insgesamt: {', '.join(returned_items) or '-'}\n"
            f"Status: {'abgeschlossen' if status == 'closed' else ('unvollständig' if status == 'partial' else 'offen')}\n"
            f"Zeitpunkt vollständige Rückgabe: {fmt_datetime(closed_at) if closed_at else '-'}\n"
            f"Bemerkung: {return_note or '-'}"
        ),
    )
...
def delete_confirm(request: Request, project_id: int):
    ...
    return templates.TemplateResponse(
        "delete_project.html",
        template_context(request, project=proj),
    )
...
def download_log(project_id: int):
    ...
    return FileResponse(
        path,
        filename=f"Log - {safe_filename(proj['name'])}.txt",
...
def export_project(project_id: int):
    ...
    with db() as con:
        loans = con.execute(
            "SELECT * FROM loans WHERE project_id=? ORDER BY sequence_no ASC",
            (project_id,),
        ).fetchall()
